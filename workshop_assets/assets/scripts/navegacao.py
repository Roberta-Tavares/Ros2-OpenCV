...
class ZigZagNavigator(Node):
    def __init__(self, linhas, colunas, espacamento):
        ...
        # Gera os waypoints em formato zigue-zague
        ...
        self.get_logger().info("🚀 Nó de navegação ZigZag iniciado.")

    def generate_zigzag_path(self):
        """Gera uma lista de coordenadas (x, y) em padrão zigue-zague"""
        path = []
    ...
    def get_yaw(self):
        """Extrai o ângulo (yaw) da orientação quaternion"""
    ...
    def timer_callback(self):
        """Controle principal de movimento"""
        ...
        if dist < 0.2:
            ...
            if self.current_target_idx >= len(self.path):
                self.get_logger().info("🏁 Todos os pontos foram visitados!")
    ...
    def image_callback(self, msg):
        """Mostra a imagem da câmera e grava o vídeo"""
        try:
